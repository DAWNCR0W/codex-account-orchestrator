...
CAO는 계정 라우팅과 계정 컨텍스트 선택을 담당합니다. CAO가 사용자 명령을 직접 실행하거나, 샌드박스 권한을 부여하거나, 승인 정책을 우회하지는 않습니다.

실제 실행 경계는 Codex 런타임이 계속 담당합니다. 여기에는 샌드박스, 파일 시스템과 네트워크 접근, 승인 정책 적용이 포함됩니다. CAO의 라우팅 결정은 어떤 계정 컨텍스트를 사용할지만 정하며, 그 컨텍스트 안에서 Codex가 할 수 있는 작업 범위를 넓히지는 않습니다.
